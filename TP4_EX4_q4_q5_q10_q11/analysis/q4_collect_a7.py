...
def main() -> None:
    ap = argparse.ArgumentParser(description="Collect Q4 A7 results from gem5 stats")
    ap.add_argument(
        "--base",
        default="/home/xfalc/arq/TP4_EX4/m5out/q4/A7",
        help="Base output directory (contains L1_*/<run>/stats.txt)",
    )
    ap.add_argument(
        "--fig-base",
        default="/home/xfalc/arq/TP4_EX4/tex/figs/q4",
    ...
    ap.add_argument(
        "--out-table-tex",
        default="/home/xfalc/arq/TP4_EX4/tex/tables/q4_a7_summary.tex",
    ...
    # Collect dijkstra and blowfish (enc+dec aggregated)
    ...
    for l1 in l1_labels:
        ...
        if e is not None and de is not None:
            ...
            # Consider the aggregated run complete only if both parts completed.
    ...
    def add_row(app: str, l1: str, m: RunMetrics) -> None:
        ...
        if app == "blowfish_total":
            # Aggregated string: require both enc/dec to be complete.
    ...
    def best_l1(app: str) -> Tuple[str, RunMetrics]:
        ...
        if not candidates:
            # Fallback: if nothing completed, rank whatever we have (still write table).
    ...
    with out_table_tex.open("w") as f:
        f.write("% Auto-generated by analysis/q4_collect_a7.py\n")
    ...
    # --- Plots: performance (cycles & IPC), memory (miss rates), branch ---
    plot_series(
        x_kb,
        series("dijkstra", "num_cycles"),
        "Dijkstra (A7) - Ciclos vs L1",
        "Ciclos (numCycles)",
        fig_base / "a7_dijkstra_cycles.png",
    )
    plot_series(
        x_kb,
        series("dijkstra", "ipc"),
        "Dijkstra (A7) - IPC vs L1",
        "IPC",
        fig_base / "a7_dijkstra_ipc.png",
    )
    plot_series(
        x_kb,
        series("dijkstra", "l1d_miss_rate"),
        "Dijkstra (A7) - Miss rate DL1 vs L1",
        "Miss rate (DL1)",
        fig_base / "a7_dijkstra_dl1_miss.png",
    )
    plot_series(
        x_kb,
        series("dijkstra", "l1i_miss_rate"),
        "Dijkstra (A7) - Miss rate IL1 vs L1",
        "Miss rate (IL1)",
        fig_base / "a7_dijkstra_il1_miss.png",
    )
    plot_series(
        x_kb,
        series("dijkstra", "l2_miss_rate"),
        "Dijkstra (A7) - Miss rate L2 vs L1",
        "Miss rate (L2)",
        fig_base / "a7_dijkstra_l2_miss.png",
    )

    plot_series(
        x_kb,
        series_bp_cond_miss("dijkstra"),
        "Dijkstra (A7) - Mispred rate (cond) vs L1",
        "Mispred rate (cond)",
        fig_base / "a7_dijkstra_bp_mispred.png",
    )

    plot_series(
        x_kb,
        series("blowfish_total", "num_cycles"),
        "Blowfish (A7) - Ciclos (enc+dec) vs L1",
        "Ciclos (enc+dec)",
        fig_base / "a7_blowfish_cycles.png",
    )
    plot_series(
        x_kb,
        series("blowfish_total", "ipc"),
        "Blowfish (A7) - IPC (enc+dec) vs L1",
        "IPC (enc+dec)",
        fig_base / "a7_blowfish_ipc.png",
    )
    plot_series(
        x_kb,
        series("blowfish_total", "l1d_miss_rate"),
        "Blowfish (A7) - Miss rate DL1 (enc+dec) vs L1",
        "Miss rate (DL1)",
        fig_base / "a7_blowfish_dl1_miss.png",
    )
    plot_series(
        x_kb,
        series("blowfish_total", "l1i_miss_rate"),
        "Blowfish (A7) - Miss rate IL1 (enc+dec) vs L1",
        "Miss rate (IL1)",
        fig_base / "a7_blowfish_il1_miss.png",
    )
    plot_series(
        x_kb,
        series("blowfish_total", "l2_miss_rate"),
        "Blowfish (A7) - Miss rate L2 (enc+dec) vs L1",
        "Miss rate (L2)",
        fig_base / "a7_blowfish_l2_miss.png",
    )

    plot_series(
        x_kb,
        series_bp_cond_miss("blowfish_total"),
        "Blowfish (A7) - Mispred rate (cond) (enc+dec) vs L1",
        "Mispred rate (cond)",
        fig_base / "a7_blowfish_bp_mispred.png",
    )
